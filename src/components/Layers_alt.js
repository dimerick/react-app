...
import L, { Point, LatLng, DivOverlay } from 'leaflet';
...
const Layers = () => {
  ...
  map.whenReady(() => {
    ...
    points.map((p, i) => {
      ...
      paths += `<path d=${path} stroke="blue" strokeWidth="3" fill="none"/>`;
    })
    ...
    lastTopLeftlatLng = new L.LatLng(bounds.getNorth(), bounds.getWest()); ////this._initialTopLeft

    shift = new L.Point(0, 0);
  ...
  const getScaleDiff = (zoom) => {
    let zoomDiff = this._groundZoom - zoom;
  ...
  const updateSvg = () => {
    ...
    let bounds = map.getBounds();
    let topLeftLatLng = new L.LatLng(bounds.getNorth(), bounds.getWest());
    ...
    let scaleDiff = getScaleDiff(zoom);
    ...
    L.DomUtil.setPosition(svg, topLeftLayerPoint);

    shift.multiplyBy(scaleDelta).add(delta);
    g.setAttribute("transform", "translate(" + shift.x + "," + this._shift.y + ") scale(" + scaleDiff + ")");
